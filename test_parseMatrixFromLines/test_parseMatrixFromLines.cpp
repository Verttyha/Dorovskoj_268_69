...
namespace testparseMatrixFromLines
{
    ...
    public:
        ...
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({}); }, L"Should throw emptyFile");
        }
        ...
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({ "1 2 3", "", "4 5 6" }); }, L"Should throw emptyLine");
        }
        ...
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({ "1 2 3", "4 a 6" }); }, L"Should throw invalidSymbol");
        }
        ...
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({ "1 2 3", "4 5x 6" }); }, L"Should throw invalidSymbol");
        }

        /// @test Проверяет, что при значении вне диапазона int выбрасывается ErrorType::outOfRange
        TEST_METHOD(NumberOutOfRangeThrows)
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({ "1 2", "9999999999999999999999" }); }, L"Should throw outOfRange");
        }

        /// @test Проверяет, что при разной длине строк выбрасывается ErrorType::notRectangular
        TEST_METHOD(DifferentLengthRowsThrows)
        {
            Assert::ExpectException<ErrorType>([] { parseMatrixFromLines({ "1 2 3", "4 5", "6 7 8" }); }, L"Should throw notRectangular");
        }
